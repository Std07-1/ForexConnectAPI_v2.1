...
> Примітка про ізоляцію середовищ: у документах часто фігурують канонічні канали `fxcm:*` (наприклад, `fxcm:ohlcv`). У локальному профілі вони можуть бути `fxcm_local:*` (через `FXCM_CHANNEL_PREFIX` / `FXCM_OHLCV_CHANNEL`). Семантика bucket-ів і формат payload-ів при цьому не змінюються.
