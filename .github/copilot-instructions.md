# Інструкція для GitHub Copilot (AiOne_*) • v3.0
...
- Завжди й кожну зміну описуй у UPDATE.md українською.
- Інколи потрібно перевірити UPDATE.md, щоб зрозуміти контекст і вимоги, та чи йде виправлення вже з минулими змінами.
...
- Конфіг стратегії та бізнес-константи — через `config`, локальні `config_*` модулі, параметри функцій.
...
- Кожен модуль має зрозумілі зони відповідальності (див. нижче).
- Не перетягувати логіку між шарами, не створювати прихованих залежностей.

### 1.5 Латентність

- У WS-потоці не додавай важкі обчислення чи блокуючі I/O.
...
- Усі розрахунки виконуються раніше. UI тільки читає поля, класифікує/сортує/фільтрує.
- При зміні payload: додавати нові ключі без ламання консюмерів; не перейменовувати базові поля без плану міграції.

### 2.4 Конфігурація (`config.py`)

- `config` — джерело правди для назв ключів/каналів Redis (namespace, UI channel, snapshot keys, TTL та інші).
- Нові бізнес-константи додавай у `config.py` або окремі `config_*` модулі, а не в ENV.
...
- Ризик-параметри продакшн-логіки не змінювати без прямої вказівки; вони мають відповідати `_*.md` або конфігам.
